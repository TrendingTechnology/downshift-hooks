...
A dropdown created with HTML elements such as `<label>` for label, `<ul>` and `<li>` for the items list, and `<button>` for the toggle button.
...
A dropdown created with StardustUI components such as `<Label>` for label, `<List>` and `<ListItem>` for the items list, and `<Button>` for the toggle button.
    ...
    const DropdownSelectionStardustUI = () => {
      ...
      return (
        <Provider theme={themes.teams}>
          ...
          <Ref innerRef={toggleButtonProps.toggleButtonRef}>
            <StardustButton
...
A dropdown created with MaterialUI components such as `<FormLabel>` for label, `<List>` and `<ListItem>` for the items list, and `<Button>` for the toggle button.
...
<Playground style={{height: '300px'}}>
  {() => {
    ...
    const DropdownSelectionMaterialUI = () => {
      const {
        isOpen,
        selectedItem,
        getToggleButtonProps,
      ...
      return (
        <>
          ...
          <Button
            ...
            ref={toggleButtonProps.toggleButtonRef}
            {...toggleButtonProps}
          >
    ...
    return <DropdownSelectionMaterialUI />
}}
