...
describe('getMenuProps', () => {
	...
	describe('hook props', () => {
		test('assign default value to aria-labelledby', () => {
			...
			expect(menuProps['aria-labelledby'])
				.toEqual(`${defaultIds.label}`)
		})
	...
	describe('user props', () => {
		...
		test('event handler onKeyDown is called along with downshift handler', () => {
			...
			act(() => {
				const { ref: menuRef, onKeyDown } = result.current.getMenuProps({ onKeyDown: userOnKeyDown })
		...
		test('event handler onBlur is called along with downshift handler', () => {
			...
			act(() => {
				const { ref: menuRef, onBlur } = result.current.getMenuProps({ onBlur: userOnBlur })
